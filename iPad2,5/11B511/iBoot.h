#define IMAGE_NAME              "iBoot.p105ap.RELEASE.dec" /* iPad mini */
#define IMAGE_START             0x9FF00000 
#define IMAGE_END               0x9FF53564
#define IMAGE_SIZE              (IMAGE_END - IMAGE_START)
#define IMAGE_HEAP_SIZE         0xA3A9C 
#define IMAGE_BSS_START         0x9FF44840 
#define IMAGE_TEXT_END          0x9FF44000  /* XXX this is a lie */
#define IMAGE_STACK_SIZE        0x1000 
#define IMAGE_LOADADDR          0x80000000
#define IMAGE_HUGECHUNK         0x3000000


#define breakpoint1_ADDR        (0x18ED8 + 1) /* ResolvePathToCatalogEntry */ 

#define fuck1_ADDR              (0x19D3A + 1) 
#define fuck2_ADDR              (0x19D50 + 1) 
#define fuck3_ADDR              (0x19E66 + 1) 
...
#define readp_ADDR              (0x19B00 + 1) 
#define get_mem_size_ADDR       (0x1F59C + 1) 
#define putchar_ADDR            (0x33904 + 1) 
#define adjust_stack_ADDR       (0x1ECE4 + 1) 
#define adjust_environ_ADDR     (0x1F1E0 + 1)
#define disable_interrupts_ADDR (0x34640 + 1) 
#define cache_stuff_ADDR        (0x22224 + 1) 
...
#define panic_ADDR              (0x20644 + 1) 
#define system_init_ADDR        (0x20730 + 1) 
#define task_create_ADDR        (0x20D60 + 1) 
#define task_start_ADDR         (0x20EC0 + 1) 
#define task_exit_ADDR          (0x20EE4 + 1) 
#define printf_ADDR             (0x3316C + 1) 
#define malloc_ADDR             (0x19B1C + 1) 
#define free_ADDR               (0x19BD0 + 1) 
#define create_envvar_ADDR      (0x18440 + 1) 
#define bcopy_ADDR              (0x33BF0) 
#define decompress_lzss_ADDR    (0x25200 + 1) 
...
real_fuck1(unsigned int r0, unsigned int r1, unsigned int r2, unsigned int r3)
{
    ...
    if (sp <= (uintptr_t)image + 0x47CC0 + 0x28 + 32 * 4) {
        unsigned int t2 = (uintptr_t)image + 0x47CC0 + 0x28 + r3 * 4;
...
real_fuck3(unsigned int r0, unsigned int r1, unsigned int r2, unsigned int r3)
{
    ...
    if (sp <= (uintptr_t)image + 0x47CC0 + 0x28 + 32 * 4) {
        ...
        eprintf("*** MEMORY ***\n");
        unsigned int i = 0x47C08;
        for (; i < 0x47C08 + 0x20 * 4; i += 0x10) {
...
[NAND] h2fmiPrintConfig:554 PPN Manufacturer ID: 45 82 60 DE 57 02
[NAND] h2fmiPrintConfig:556 PPN Controller Version: 040471P_SNDSK_19


=======================================
::
:: iBoot for p105ap, Copyright 2013, Apple Inc.
::
::    BUILD_TAG: 9ff55780
::
::    BUILD_STYLE: 9ff57514
::
::    USB_SERIAL_NUMBER: CPID:8942 CPRV:10 CPFM:03 SCEP:10 BDID:0A ECID:000003E1AA1183F1 IBFL:1B SRNM:[]
::
=======================================
...
[NAND] s_cxt_boot:88 sftl: error, unclean shutdown; adopted 18780 spans
...
{
#if 1
    CALL(create_envvar)("boot-ramdisk", "/a/b/c/d/e/f/g/h/i/j/k/l/m/n/o/p/q/r/s/t/u/disk.dmg", 0);
#endif
}
...
#define SHELLCODE_BASE 0x47C08
#define RAMDISK_START 0x47790
#define BTREE_HEADER (0x47AA0 + 0x14)
...
if (1) {
    ...
    switch (seq) {
        ...
        case 2:
            ...
            PUT_DWORD_LE(buffer, SHELLCODE_BASE +  50 - EXTENTS_BTREE_HEADER, make_bl(0, SHELLCODE_BASE + 50, 0x20D54));
            ...
            PUT_DWORD_LE(buffer, SHELLCODE_BASE + 100 - EXTENTS_BTREE_HEADER, 0x418B8 );
            PUT_DWORD_LE(buffer, SHELLCODE_BASE + 104 - EXTENTS_BTREE_HEADER, 0x1A778 );
...
{
    eprintf("irq_disable_count = 0x%x\n", *(unsigned int *)(image + 0x55780 + 0x18));
}
...
dispatch_SEGV(void *si_addr, _STRUCT_ARM_THREAD_STATE *tctx)
{
    ...
    if (si_addr == 0) {
        if (tctx->__pc == (uintptr_t)(image + 0x20B72)) {
            /* idle task crap (read from *0) */
            tctx->__r[0] = *(uint32_t *)(image);
            tctx->__pc += 2;
            return 0;
        }
        if (tctx->__pc == (uintptr_t)(image + 0x20DD2)) {
...
patch_image(unsigned char *image)
{
    ...
    *(uint32_t *)(image + 0x2073C) = INSN2_LDR_R1_PC__B_PLUS4; 
    *(void **)(image + 0x20740) = XLAT(IMAGE_END + IMAGE_HEAP_SIZE); 
    /* clean data cache */
    *(uint32_t *)(image + 0x21E28) = INSNA_RETURN; 
#if !USE_SIGNAL
    /* idle task crap (read from *0) */
    *(uint16_t *)(image + 0x20B72) = INSNT_NOP; 
    *(uint16_t *)(image + 0x20DD2) = INSNT_MOV_R_R(1, 0); 
#endif /* !USE_SIGNAL */
    /* timer 2 */
    *(uint32_t *)(image + 0x1FEE8) = INSN2_RETURN_0; 
    /* task switch FPU */
    *(uint32_t *)(image + 0x22258) = INSNA_MOV_R2_0; 
    *(uint32_t *)(image + 0x22288) = INSNA_MOV_R2_0; 
    /* bzero during mmu_init */
    *(uint32_t *)(image + 0x2130C) = INSN2_NOP__NOP; 
    ...
    *(uint32_t *)(image + 0x19736) = INSN2_MOV_R0_1__MOV_R0_1; 
    ...
    /* 将真实iboot也做对应修改，再kloader运行，得到数据后用adjust_stack对齐 */
    *(uint16_t *)(image + 0xD94) = INSNT_MOV_R_R(1, 13); 
    *(uint8_t *)(image + 0x34A24) = 'x'; 
    /* show task structure */
    *(void **)(image + 0xFA4) = image + 0x44520; 
    *(uint8_t *)(image + 0x34A10) = 'x'; 
    ...
    *(uint32_t *)(image + 0x026D0) = INSN2_RETURN_0;
    /* nop 0x3fa00094 */ 
    *(uint32_t *)(image + 0x02D34) = INSN2_RETURN_0; 
    *(uint32_t *)(image + 0x02D74) = INSN2_RETURN_0; 

    *(uint32_t *)(image + 0x00D60) = INSN2_NOP__NOP;
    *(uint16_t *)(image + 0x0C180) = INSNT_NOP; /* XXX loop */
    *(uint16_t *)(image + 0x200A2) = INSNT_NOP;
#if 0
  *(uint16_t *)(image + 0xXXXXX) = INSNT_NOP;
#endif
    *(uint32_t *)(image + 0x1F410) = INSN2_RETURN_0;
    *(uint32_t *)(image + 0x1F612) = INSN2_RETURN_0;
    *(uint32_t *)(image + 0x1F632) = INSN2_RETURN_0;
    *(uint32_t *)(image + 0x1F66C) = INSN2_RETURN_0;
    *(uint32_t *)(image + 0x1F684) = INSN2_RETURN_21;
    *(uint32_t *)(image + 0x1F69C) = INSN2_RETURN_0;
    /* nocache */
#if 0
    *(uint32_t *)(image + 0x18850) = INSNT_RETURN;
#endif
}
